...
* Verwenden Sie die `ABRControlParameterBuilder` Helper-Klasse, um eine beliebige Untergruppe der Parameter festzulegen (funktioniert hinter den Kulissen `ABRControlParameter` )

* Legen Sie die Parameter für die `ABRControlParameter` Klasse fest.

## Adaptive Bitraten mit ABRControlParametersBuilder konfigurieren {#section_3DDE397A7CE445E1832EBAA46CE5C069}

Die Verwendung der `ABRControlParametersBuilder` Helper-Klasse ist die einfachste und effizienteste Methode zum Festlegen von ABR-Parametern.

* Der `ABRControlParametersBuilder` Konstruktor setzt alle ABR-Parameter auf Standardwerte für das zugrunde liegende `ABRControlParameters` Objekt.
...
Diese Klasse umfasst auch die `toABRControlParameters()` Helper-Methode. Verwenden Sie diese Methode, um eine Instanz von abzurufen `ABRControlParameters` und sie für die `mediaPlayer.ABRControlParameters` Eigenschaft festzulegen. Dadurch werden Ihre Einstellungen im Player wirksam.

1. Instanziieren Sie die `ABRControlParametersBuilder` Helper-Klasse und legen Sie die Parameter für den Media Player fest.
...
## Adaptive Bitraten mit ABRControlParameters konfigurieren {#section_02161FD0A73F40ED9CAE17F9AF850483}

Sie können ABR-Steuerelementwerte nur mit festlegen, `ABRControlParameters`aber jederzeit eine neue erstellen.

Diese Fähigkeit zum Festlegen von ABR-Parametern wurde vor dem Vorhandensein der `ABRControlParametersBuilder` Klasse unterstützt, aber diese Fähigkeit ist zum Festlegen von ABR-Parametern zur Bauzeit noch immer effektiv. Um jedoch einzelne Parameter nach der Erstellung zu ändern, sollten Sie die `ABRControlParametersBuilder` Klasse verwenden.
