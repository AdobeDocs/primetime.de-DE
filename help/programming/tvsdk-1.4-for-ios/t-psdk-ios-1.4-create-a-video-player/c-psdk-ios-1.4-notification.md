...
Wenn `PTMediaPlayer` eine Eigenschaft des Client-Players ist, stellt `self.player` im folgenden Beispiel die `PTMediaPlayer`-Instanz dar. Im folgenden Beispiel wird die `addObservers`-Methode implementiert, die in den PTMediaPlayer-Setupanweisungen angezeigt wird und die meisten Benachrichtigungen enthält:
...
<table frame="all" colsep="1" rowsep="1" id="table_ios_notifications"> 
 <tbody> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> Ein anderes <span class="codeph"> PTMediaPlayerItem </span> von <span class="codeph"> PTMediaPlayer </span> wurde eingestellt. </td> 
  </tr> 
  <tr rowsep="1"> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> Eine neue <span class="codeph"> PTNotification </span> wurde dem <span class="codeph"> PTNotificationHistoryItem </span> des aktuellen <span class="codeph"> PTMediaPlayerItem </span> hinzugefügt, d. h. wenn ein Benachrichtigungs-Ereignis zum Benachrichtigungsverlauf hinzugefügt wird. </td> 
  </tr> 
  <tr rowsep="1"> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> <p>Ein abonniertes Tag wird im Hintergrundmanifest identifiziert und eine neue <span class="codeph"> PTTimedMetadata </span>-Instanz wird darauf vorbereitet. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
Rufen Sie die `PTAdBreak`-Instanz mit `PTMediaPlayerAdBreakKey` ab:
...
Legen Sie `subtitlesOptions` und `audioOptions` fest:
...
Rufen Sie die `PTAd`-Instanz mit `PTMediaPlayerAdKey` ab:
