...
Nachdem die Primetime-Authentifizierungs-OLCA-Konformität beschrieben und die Adobe-Teilnahme an OATC vorgenommen wurde, ist es auch wichtig festzustellen, dass die Primetime-Authentifizierung tatsächlich &quot;protokollagnostisch&quot;ist.  Aber in dieser Phase der TVE-Ära orientiert sich Primetime-Authentifizierung definitiv an den OLCA-Standards.  In den Standards werden derzeit vereinbarte Methoden zur Implementierung von TVE-Funktionen durch die verschiedenen TVE-Player (Programmierer, MVPDs und Service Provider) definiert. Viele dieser Funktionen sind in den folgenden Tabellen aufgeführt, einschließlich Links zu verwandten Seiten, die Details und Beispiele zur Implementierung der Funktionen enthalten.
...
| 1.19 | Dynamisch definierter TTL-Wert (Token Persistence Time-to-Live) | Der MVPD kann die TTL des Primetime-Authentifizierungstokens dynamisch über Eigenschaften in der Antwort festlegen, sodass Adobe bei TTL-Änderungen nicht in der Warteschlange ist. | C- |                                                                                                                                                         |
