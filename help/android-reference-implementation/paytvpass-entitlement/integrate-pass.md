---
description: Passen Sie Ihre Referenzimplementierung an, um die Adobe Primetime-Authentifizierung für Ihre Produktions-Umgebung zu integrieren.
seo-description: Passen Sie Ihre Referenzimplementierung an, um die Adobe Primetime-Authentifizierung für Ihre Produktions-Umgebung zu integrieren.
...
Passen Sie Ihre Referenzimplementierung an, um die Adobe Primetime-Authentifizierung für Ihre Produktions-Umgebung zu integrieren.
...
1. Aktivieren oder deaktivieren Sie Berechtigungsabläufe.
   ...
   Der `EntitlementManager` muss zuerst initialisieren und eine Instanz des Primetime-Authentifizierungs-SDK abrufen, damit sie aktiviert werden kann. Wenn die Bibliothek `EntitlementManager` nicht initialisiert wird, wird der Manager deaktiviert.
...
1. Verwenden Sie die `ManagerFactory` Klasse, um eine Instanz der `EntitlementManager`zu erhalten.

   Sie müssen immer die verwenden, `ManagerFactory` um eine Instanz der zu erhalten, `EntitlementManager`da die Instanz von EntitlementManager nur eine Instanz von EntitlementManager für Ihre Anwendung `ManagerFactory` verwaltet. Instanziieren Sie niemals die `EntitlementManager` oder `EntitlementManagerOn` Klassen mit ihren Konstruktoren.
   ...
   Der `ManagerFactory` gibt eine Instanz von zurück, `EntitlementManagerOn`bei der die Berechtigungsabläufe aktiviert sind, wenn Sie zuvor aufgerufen haben `EntitlementManager.initializeAccessEnabler`. Wenn Sie nicht zum ersten Mal aufrufen `EntitlementManager.initializeAccessEnabler`, gibt der `ManagerFactory` Benutzer eine Instanz von zurück, `EntitlementManager`wobei die Berechtigungsabläufe deaktiviert sind. 1. Konfigurieren Sie die Anforderungs-ID.

   Die Referenz-Implementierung ist vorkonfiguriert und die Test-Anforderer-ID ist auf Folgendes eingestellt: &quot;REF&quot;. Mit dieser Anforderungs-ID können Sie Ihre Anwendung testen. Wenn Sie zur Verwendung der Anforderungs-ID bereit sind, die Ihnen Ihr Primetime-Authentifizierungsbeauftragter zur Verfügung stellt, aktualisieren Sie die [!DNL res/values/strings.xml] Datei der Anwendung mit Ihrer Anforderer-ID.
...
1. Konfigurieren Sie die Server-Umgebung.
   ...
   * Produktion - Die Produktions-Umgebung wird für Live-Implementierungen Ihrer Applikation verwendet.
   Sie legen die URIs sowohl für die Staging- als auch für die Produktions-Umgebung mithilfe der Anwendung fest. Sie müssen jedoch festlegen, welche dieser URIs von der Anwendung im Code verwendet werden. Legen Sie in der `com.adobe.primetime.reference.manager.EntitlementManger` Klasse für die `environmentUri` Variable je nach verwendeter Primetime-Authentifizierungsdienst-Umgebung entweder `STAGING_URI` oder `PRODUCTION_URI` fest.
...
1. Passen Sie das MVPD-Auswahlraster an.
   ...
   Auf der Seite zur Auswahl des Content Providers wird eine Tabelle der neun wichtigsten MVPDs angezeigt, aus denen der Benutzer auswählen kann. Die Anwendung ruft die neun wichtigsten MVPDs aus einer bestellten Liste innerhalb der Anwendung ab, die mit den verfügbaren MVPDs übereinstimmt, die mit dem Programmierer im Primetime-Authentifizierungssystem integriert sind. Die bestellte Liste der primären MVPDs wird auf der MVPD-ID innerhalb des Primetime-Authentifizierungs-Systems und nicht auf dem MVPD-Anzeigenamen gezählt. Es ist wichtig zu überprüfen, ob die MVPD-IDs in der primären MVPD-Liste mit den MVPD-IDs übereinstimmen, die mit dem Programmiererkonto integriert sind, da die IDs in einigen Fällen von Integrationen verschieden sein können. Nachstehend finden Sie die geordnete Liste der primären MVPDs, die in der Klasse gefunden werden `com.adobe.primetime.reference.ui.entitlement.MvpdPickerFragment`.
